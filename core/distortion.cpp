...
namespace core {
  ...
  const char * show_vr_compositor_error(EVRCompositorError e) {
    switch (e) {
  ...
  openvr_distortion::openvr_distortion(const core::window & window, openvr_tracker & tracker, float nearZ, float farZ)
    : window(window)
    , tracker(tracker)
    , ipd_connection(tracker.on_ipd_changed.connect([&] { recalculate_pose(); }))
    , shader("distortion",
      R"(#version 410 core
