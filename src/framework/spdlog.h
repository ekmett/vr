...
namespace framework {
  ...
  namespace logging {
    ...
    // "template-typedefs"
    template <typename ... T> using squelched_sink_mt = details::squelched_sink<std::mutex, T...>;
    template <typename ... T> using squelched_sink_st = details::squelched_sink<spdlog::details::null_mutex, T...>;

    extern shared_ptr<logger> default_factory(const char * name);
    extern shared_ptr<logger>(*factory)(const char * name);
    inline shared_ptr<logger> log(const char * name) {
