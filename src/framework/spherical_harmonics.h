...
namespace framework {
    ...
    template <typename T, size_t N> struct sh : array<T, N> {
      ...
      inline auto map(const sh<T,N> & ... args, F f) -> sh<decltype(f(elems[0], args[0]...)),N> const {
        sh<decltype(f(elems[0], args[0]...)), N> result{};
        for (size_t i = 0; i < N; ++i) result[i] = f(elems[i], args[i]...);
        return result;
      }

      // in place modification
      template <typename F, typename ... T> inline sh & modify(const sh<T, N> & ... args, F f) {
        for (size_t i = 0;i < N; ++i) f(elems[i], args[i]...);
        return *this;
      }
      ...
      inline sh operator+(const sh & that) const {
        sh result;
        for (size_t i = 0; i < N; ++i) result[i] = elems[i] + that[i];
        return result;
      }
      inline sh & operator-=(const sh & that) {
        for (size_t i = 0; i < N; ++i) elems[i] -= that[i];
      ...
      inline sh operator*=(const T& scale) {
        for (size_t i = 0; i < N; ++i) elems[i] *= scale;
        return *this;
      }
      inline sh operator*(const T& scale) const {
        sh result;
      ...
      template <typename U> 
      auto dot(const sh<U,N> & that) -> decltype(elems[0] * that.elems[0]) const {
        decltype(elems[0]*that.elems[i]) result;
        for (size_t i = 0; i < N; ++i) result += elems[i] * that.elems[i];
        return result;
      }
      ...
      sh convolved_with_cos_kernel() {
        ...
        sh result{};
        if (N > 0) result[0] = elem[0] * cosA0;
        for (int i = 0;i < min(4, N);++i) result[i] = elem[i] * cosA1;
        for (int i = 4;i < min(9, N);++i) result[i] = elem[i] * cosA2;
        // truncate any remaining terms for now
        return result;
      }

    };
    template <typename OStream, typename T, size_t N> static inline OStream & operator<<(OStream & os, const sh<T, N> & s) {
      os << "sh {";
      if (N > 0) 
        os << " " << s[0];
      for (int i = 1; i < N; ++i)
